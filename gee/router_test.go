...
func TestGetRoute(t *testing.T){
	r := newTestRouter()
	n, ps := r.getRoute("GET", "hello/justniu")

	if n == nil {
		t.Fatal("nil shoouldn't be returned")
	}
	...
	if ps["name"] != "justniu" {
		t.Fatal("name should be equal to 'justniu'")
	}
...
func TestGetRoute2(t *testing.T){
	...
	if !ok1 {
		t.Fatal("pattern should be /asset/*filepath & filepath should be file1.txt")
	}
